...
def read_GDSC_response(GDSC_drug_response_frames, GDSC_drug_name, X_source,
                    unique_drugs=None, GDSC_drug_id=None, metric='AUC'):
    # X_source has to be a DataFrame with genes in columns

    if unique_drugs is not None:
        if GDSC_drug_name in unique_drugs['GDSC2']:
            GDSC_drug_response_df = GDSC_drug_response_frames['GDSC2'].copy()
        elif GDSC_drug_name in unique_drugs['GDSC1']:
    ...
    y_source = y_source[['CELL_LINE_NAME', metric]]
